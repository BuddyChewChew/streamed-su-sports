...
function discoverListings() {
    ...
    while ($attempt < $maxRetries && !$success) {
        $attempt++;
        $html = file_get_contents('https://rr.vipstreams.in/s/');
        ...
        if (json_last_error() === JSON_ERROR_NONE) {
            if (isset($jsonData[1]['data']['liveMatches'])) {
                ...
                foreach ($allMatches as $match) {
                    ...
                    $poster = 'https://raw.githubusercontent.com/BuddyChewChew/streamed-su-sports/main/images/sports.png';
					
                    if (stripos($match['category'], 'afl') === 0) {
                        $poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/afl.png?raw=true';
                    }
					if (stripos($match['category'], 'american-football') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/am-football.png?raw=true';
					}
					if (stripos($match['category'], 'baseball') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/baseball.png?raw=true';
					}
					if (stripos($match['category'], 'basketball') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/basketball.png?raw=true';
					}
					if (stripos($match['category'], 'billiards') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/billiards.png?raw=true';
					}
					if (stripos($match['category'], 'cricket') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/cricket.png?raw=true';
					}	
					if (stripos($match['category'], 'darts') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/darts.PNG?raw=true';
					}	
					if (stripos($match['category'], 'football') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/football.png?raw=true';
					}					
					if (stripos($match['category'], 'fight') === 0) {
						$poster = 'https://raw.githubusercontent.com/BuddyChewChew/streamed-su-sports/main/images/fighting.png';
					}	
					if (stripos($match['category'], 'golf') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/golf.png?raw=true';
					}	
					if (stripos($match['category'], 'hockey') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/hockey.png?raw=true';
					}	
					if (stripos($match['category'], 'motor-sports') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/motor.png?raw=true';
					}	
					if (stripos($match['category'], 'nba') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/nba.png?raw=true';
					}	
					if (stripos($match['category'], 'rugby') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/rugby.png?raw=true';
					}		
					if (stripos($match['category'], 'tennis') === 0) {
						$poster = 'https://github.com/BuddyChewChew/streamed-su-sports/blob/main/images/tennis.png?raw=true';
					}						
...
function generateM3U8($items) {
    $m3u8 = "#EXTM3U url-tvg=\"https://raw.githubusercontent.com/BuddyChewChew/streamed-su-sports/main/epg.xml\"\n";
...
function generateVLC($items) {
	$vlc = "#EXTM3U url-tvg=\"https://raw.githubusercontent.com/BuddyChewChew/streamed-su-sports/main/epg.xml\"\n";
    foreach ($items as $item) {
        $date = new DateTime("@".($item['time'] / 1000));
        $date->setTimezone(new DateTimeZone('America/New_York'));
        $formattedTime = $date->format('h:i A -');
...
function generateKODIPOP($items) {
	$kodipop = "#EXTM3U url-tvg=\"https://raw.githubusercontent.com/BuddyChewChew/streamed-su-sports/main/epg.xml\"\n";
    foreach ($items as $item) {
        $date = new DateTime("@".($item['time'] / 1000));
        $date->setTimezone(new DateTimeZone('America/New_York'));
        $formattedTime = $date->format('h:i A -');
